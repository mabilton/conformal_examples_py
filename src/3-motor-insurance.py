...
def __(Path, mo, np, pd, requests, warnings):
    def load_fremtpl2_dataset(
        ...
        df = df_freq.merge(df_sev, how='outer', on='IDpol', validate='one_to_one', indicator=True)
        unmatched_claims = df['_merge'] == 'right_only'
        if verbose and np.any(unmatched_claims):
            num_unmtch_policies = int(unmatched_claims.sum())
            num_unmtch_claims = int(df.loc[unmatched_claims, 'Claims'].apply(lambda x: len(x)).sum())
            warnings.warn(
        ...
        df['NumClaims'] = df['Claims'].apply(lambda x: len(x) if isinstance(x, list) else 0)
        df['TotalLoss'] = df['Claims'].apply(lambda x: sum(x) if isinstance(x, list) else 0)
    ...
    def download_fremtpl2_dataset(save_dir: Path | str | None = None, chunk_size: int = 8192) -> dict[str, Path]:
                ...
                try:
                    with requests.get(dataset_url, stream=True) as response:
                        ...
                        with mo.status.progress_bar(
                            total=total_size,
                            title=f'Downloading `{str(filename)}`...'
                        ) as pbar:
...
def __(alt, explode_claims, np, pd):
    def plot_fremtpl2_freq_dist(data: pd.DataFrame):
        freq_min, freq_max = data['ClaimNb'].min(), data['ClaimNb'].max()
        ...
        df_freq = pd.DataFrame(
            {
                'freq': freq,
                'log10_count': np.log10(freq_counts)
            }
        )
        freq_plot = alt.Chart(df_freq).mark_bar().encode(
            x=alt.X('freq:Q', title='Number of Claims against Policy', scale=alt.Scale(domain=[freq.min(), freq.max()])),
    ...
    def plot_fremtpl2_sev_data(data: pd.DataFrame, epsilon: float = 1e-5, nbins: int = 30) -> alt.Chart:
        claim_amts = explode_claims(data)['ClaimAmount'].to_numpy(dtype=float)
        hist_values, bin_edges = np.histogram(np.log10(claim_amts + epsilon), bins=nbins)
        ...
        sev_plot = alt.Chart(df_sev).mark_bar().encode(
            x=alt.X(
                'bin_start:Q', 
...
def __(data, plot_fremtpl2_freq_dist, plot_fremtpl2_sev_data):
    ...
    plot_fremtpl2_sev_data(data).properties(
        width=400,
        height=300
    )
    return
...
def __(np, pd, train_test_split):
    def train_test_calib_valid_split(
        ...
        quantiles = qcut_with_tie_breaking(df[stratify].to_numpy(), nbins=nbins, rng=rng)
        ...
        df_train, df_not_train, _, quantiles_not_train = train_test_split(
            df, quantiles, train_size=train_size, stratify=quantiles, random_state=sample_random_seed(rng)
        )
    ...
    def qcut_with_tie_breaking(x: np.ndarray, nbins: int, rng: np.random.Generator, epsilon: float = 1e-5) -> np.ndarray:
        tie_breaking_noise = sample_tie_breaking_noise(size=x.size, rng=rng, epsilon=epsilon)
        return pd.qcut(x + tie_breaking_noise, q=nbins, labels=False)

    def sample_tie_breaking_noise(size: int, rng: np.random.Generator, epsilon: float = 1e-5):
        ...
        **gbm_params
        ...
        feature_preprocessor = ColumnTransformer(
            [
                ...
                ("numeric", StandardScaler(), ["VehAge", "DrivAge", "BonusMalus", "Density"]),
    ...
    def optimize_gbm_model_hyperparameters(
        ...
        def objective_fn(trial: optuna.Trial) -> float:
            gbm_params = {
                ...
                "l2_regularization": trial.suggest_float("l2_regularization", 1e-3, 0.1, log=True),
            }
            ...
            if model_type == 'freq':
                test_loss = root_mean_squared_error(test_data['ClaimNb'], test_data['Exposure']*y_pred)
            elif model_type == 'sev':
        ...
        try:
            optuna.logging.set_verbosity(optuna.logging.WARNING if not verbose else current_verbosity)
...
def __(Literal, alt, np, pd, qcut_with_tie_breaking):
    def plot_model_residuals(
        ...
        scatter_plot = alt.Chart(df_samples).mark_circle(
            size=marker_size,
        ...
        hline_plot = alt.Chart(df_hline).mark_rule(
    ...
    def plot_total_response_by_pred_quantile(
        ...
        y_pred_quantiles = qcut_with_tie_breaking(y_pred, nbins=nquantiles, rng=np.random.default_rng(seed))
        df = pd.DataFrame(
            {
                'y_pred': y_pred,
                'y_obs': y_obs,
        ...
        plot = alt.Chart(df_plot).mark_line(
...
def __(df_train, df_valid, freq_train_pred, freq_valid_pred):
    print(f'Predicted total number of claims (training data): {freq_train_pred.sum().astype(int):,}')
    print(f'Observed total number of claims (training data): {df_train["ClaimNb"].sum():,}')
    print(f'Predicted total number of claims (validation data): {freq_valid_pred.sum().astype(int):,}')
    print(f'Observed total number of claims (validation data): {df_valid["ClaimNb"].sum():,}')
    return
...
def __(
    ...
    plot_total_response_by_pred_quantile(
        y_pred=sev_train_pred,
        y_obs=df_train_claims['ClaimAmount'],
        model_type='sev'
    ).properties(
...
def __(df_train_claims, df_valid_claims, sev_train_pred, sev_valid_pred):
    ...
    print(f'Observed total loss (training data): {df_train_claims["ClaimAmount"].sum():,.2f}')
    print(f'Predicted total loss (validation data): {sev_valid_pred.sum():,.2f}')
    print(f'Observed total loss (validation data): {df_valid_claims["ClaimAmount"].sum():,.2f}')
    return
...
def __(np, sample_tie_breaking_noise):
    def calibrate_regressor(
        ...
        calib_scores = calib_scores + sample_tie_breaking_noise(size=calib_scores.size, rng=rng)
...
def __(Literal, alt, np, pd, qcut_with_tie_breaking):
    def plot_prediction_intervals_with_validation_points(
        ...
        y_lb: np.ndarray,
        y_ub: np.ndarray,
        model_type: Literal['freq', 'sev', 'loss'],
        sampling_frac: float | None = None,
        seed: int | None = None,
        ...
        is_covered = (
            (df_samples['y_lb'] <= df_samples['y_obs']) 
            & 
...
def __(compute_coverage, df_valid_claims, sev_valid_lb, sev_valid_ub):
    sev_coverage_valid = compute_coverage(
        y_valid=df_valid_claims['ClaimAmount'],
        y_lb=sev_valid_lb,
        y_ub=sev_valid_ub
    )
...
def __(PolicyTotalLossModel, model_freq, model_sev):
    model_loss = PolicyTotalLossModel(
        freq_model=model_freq,
        sev_model=model_sev
    )
    return (model_loss,)
