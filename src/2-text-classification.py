...
def __(Path, mo, pd, requests):
    def load_news_classification_dataset(
        categories: list[str] | None = None, num_categories: int | None = None, save_dir: Path | str | None = None
    ) -> pd.DataFrame:
    ...
    def download_news_classification_dataset(save_dir: Path | str | None = None, chunk_size: int = 8192) -> Path:
...
def __(LabelEncoder, load_news_classification_dataset):
    num_categories = 10
    data = load_news_classification_dataset(
        num_categories=num_categories
    )

    # Drop columns we won't analyse:
    data = data.drop(
        columns=['link', 'authors']
    )
...
@app.cell(hide_code=True)
def __(pd):
    def preview_news_dataset_label_distribution(data: pd.DataFrame, pretty_formatting: bool = True) -> pd.DataFrame:
...
def __(np, pd, train_test_split):
    def train_test_calib_valid_rest_split(
        ...
        df_train, df_not_train = train_test_split(
            df, train_size=train_size, stratify=df[stratify], random_state=sample_random_seed(rng)
        )
...
def __(Pipeline, np):
    def calibrate_model(
        ...
        scores = 1 - np.take_along_axis(arr=logits, indices=y_calib.reshape(-1,1), axis=1).reshape(-1)
...
def __(accuracy_score, df_test, df_train, model):
    accuracy_train = accuracy_score(y_pred=model.predict(df_train['X']), y_true=df_train['y'])
    accuracy_test = accuracy_score(y_pred=model.predict(df_test['X']), y_true=df_test['y'])
...
def __(compute_prediction_mask, logits_valid, qhat):
    pred_masks_valid = compute_prediction_mask(
        logits=logits_valid,
        qhat=qhat
    )
...
def __(LabelEncoder, compute_prediction_set, np, pd):
    def summarize_prediction_sets(
        ...
        df['y_true_in_set'] = df.apply(lambda row: row['y_true'] in row['pred_set'], axis=1)
        df = df[['X', 'logits', 'y_pred', 'pred_set', 'y_true', 'pred_set_size', 'y_true_in_set']]
        return df
...
def __(alt, np, pd):
    def plot_article_date_distribution(dates: pd.Series, categories: pd.Series):
        df = pd.DataFrame(
            {
                'date': dates,
                'category': categories.str.title()
            }
        )
        df['month'] = df['date'].dt.to_period('M')
        df_monthly = df.groupby(['month', 'category']).size().reset_index(name='count')
        df_monthly_totals = df_monthly.groupby('month')['count'].sum().reset_index(name='total')
        ...
        num_articles_plot = alt.Chart(df_monthly_totals).mark_area().encode(
            x=alt.X('month:T', title='Date'),  # Adjust maxbins for bin granularity
            y=alt.Y('cumsum:Q', title='Cumulative Number of Articles')
        ).properties(
            title='Cumulative Number of Articles Published over Time'
        )

        category_area_plot = alt.Chart(df_monthly).mark_area().encode(
            x=alt.X('month:T', title='Date'),
            y=alt.Y('frac:Q', stack='normalize', title='Fraction of Categories'),
            color=alt.Color('category:N', title='Category')
        ).properties(
            title='Fraction of News Categories over Time'
        )
...
def __(data, plot_article_date_distribution):
    plot_article_date_distribution(
        dates=data['date'],
...
def __(np):
    def compute_coverage(prediction_mask: np.ndarray, y_true: np.ndarray) -> float:
        is_covered = np.take_along_axis(arr=prediction_mask, indices=y_true.reshape(-1,1), axis=1).reshape(-1)
...
def __(
    ...
    def sample_empirical_coverage_distribution(
        ...
        for batch_idx in mo.status.progress_bar(range(num_samples), total=num_samples, title='Sampling Empirical Coverages...'):
            ...
            pred_mask_valid = compute_prediction_mask(
                logits=logits_valid,
                qhat=qhat
            )
            ...
            result_i = {
                'coverage': compute_coverage(
                                prediction_mask=pred_mask_valid,
                                y_true=y_valid
                            ),
...
def __(betabinom, np, warnings):
    def theoretical_finite_sample_coverage_pmf(covered_size: np.ndarray, alpha: np.ndarray, calib_size: np.ndarray, valid_size: np.ndarray) -> np.ndarray:
...
def __(alt, np, pd, theoretical_finite_sample_coverage_pmf):
    def plot_observed_coverage_vs_theory(
        ...
        hist_values, bin_edges = np.histogram(observed_coverage, bins=hist_nbins, density=True)
        ...
        hist = alt.Chart(df_hist).mark_bar(
        ...
        line = alt.Chart(df_pdf).mark_line(
        ).encode(
...
def __(Any, compute_coverage, np):
    def compute_class_conditional_coverage(prediction_mask: np.ndarray, y_true: np.ndarray, labels: list[Any]) -> np.ndarray:
...
def __(Any, alt, chain, np, pd):
    def plot_label_conditional_quantity(
        ...
        if two_bar_sets:
            ...
            groups_both = (len(labels)*[uncond_quantity_label]) + (len(labels)*[cond_quantity_label])
            df = pd.DataFrame(
                {
                    'label': labels_both,
                    'values': values_both,
            ...
            df = pd.DataFrame(
                {
                    'label': labels,
                    'values': uncond_quantity if uncond_quantity is not None else cond_quantity
                }
            )
        ...
        if two_bar_sets:
            bar_color = alt.Color(
                ...
                scale=alt.Scale(
                    domain=[uncond_quantity_label, cond_quantity_label]
                )
            )
        ...
        tooltips = [
            ...
            alt.Tooltip('values:Q', title=quantity_name.title(), format=quantity_format_str),
        ]
        if two_bar_sets:
            tooltips.append(
                alt.Tooltip('group:N', title='Legend')
            )
        ...
        for sizes_name, sizes in sizes_dict.items():
            if sizes is not None:
                ...
                bar_labels = alt.Chart(df_sizes).mark_text(
...
def __(
    ...
    plot_label_conditional_quantity(
        ...
        title='Label Conditional Coverage',
        height=400,
        width=800
    )
    return
...
def __(Any, np):
    ...
    def compute_label_conditional_avg_set_size(prediction_mask: np.ndarray, y_true: np.ndarray, labels: list[Any]) -> np.ndarray:
        avg_set_sizes: list[float] = []
        for label in labels:
            label_mask = (y_true == label)
            avg_set_sizes.append(
                compute_avg_set_size(prediction_mask[label_mask,:])
            )
