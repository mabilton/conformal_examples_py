...
def __():
    ...
    from scipy.stats import beta, betabinom, norm
...
def __(beta, np, warnings):
    def theoretical_infinite_sample_coverage_pdf(
        covered_frac: np.ndarray, alpha: np.ndarray, calib_size: np.ndarray
    ) -> np.ndarray:
...
def __(betabinom, np, warnings):
    def theoretical_finite_sample_coverage_pdf(
...
def __(alt, np, pd, theoretical_finite_sample_coverage_pdf):
    def plot_theoretical_finite_sample_coverage_pdf(
        alpha: float, calib_size: int, valid_size: int
    ) -> alt.Chart:
        k_values = np.arange(0, valid_size + 1)
        probabilities = [
            theoretical_finite_sample_coverage_pdf(
                k, alpha=alpha, valid_size=valid_size, calib_size=calib_size
            )
            for k in k_values
        ]
        ...
        chart = (
            ...
            .encode(
                ...
                tooltip=[
                    alt.Tooltip("k:Q", title="Number of Samples"),
                    alt.Tooltip("Probability:Q", title="Probability Mass"),
                ],
            )
            .properties(
                title=[
...
def __(alt, np, pd, theoretical_infinite_sample_coverage_pdf):
    def plot_theoretical_infinite_sample_coverage_pdf(
        alpha: float, calib_size: int
    ) -> alt.Chart:
        covered_frac = np.linspace(0, 1, 500)
        pdf_values = theoretical_infinite_sample_coverage_pdf(
            covered_frac, alpha=alpha, calib_size=calib_size
        )
        df = pd.DataFrame({"covered_frac": covered_frac, "pdf": pdf_values})
        chart = (
            alt.Chart(df)
            .mark_line()
            .encode(
                x=alt.X(
                    "covered_frac:Q", title="Coverage", axis=alt.Axis(format=".0%")
                ),
...
@app.cell
def __():
    plot_params_1 = {"alpha": 0.1, "calib_size": 50, "valid_size": 50}
...
def __(
    ...
    plot_theoretical_finite_sample_coverage_pdf(
        ...
        alpha=plot_params_1["alpha"], calib_size=plot_params_1["calib_size"]
    )
    return
...
@app.cell(hide_code=True)
def __(alt, np, pd, theoretical_infinite_sample_coverage_pdf):
    def plot_theoretical_infinite_sample_coverage_pdf_by_calib_size(
        alpha: float,
        ...
        chart = (
            ...
            .encode(
                ...
                color=alt.Color(
                    "calib_size:N",
                    legend=alt.Legend(title="Calibration Size", format=","),
                ),
            )
            .properties(
                title=[
...
@app.cell
def __(np):
    def sample_from_true_distribution(
...
def __(
    ...
    def split_conformal_prediction_with_trained_model(
        ...
        qhat = conformal_calibrate_model(
            model=model, X_calib=X_calib, y_calib=y_calib, alpha=alpha
        )
...
def __(Pipeline, alt, np, pd):
    def plot_model_with_prediction_interval(
        ...
        df = pd.DataFrame(
            {"X": X.flatten(), "y": y_pred, "lb": y_pred - qhat, "ub": y_pred + qhat}
        )
        ...
        interval = base.mark_area(opacity=opacity).encode(
            y="lb:Q", y2="ub:Q", color=alt.value(interval_color)
        )
...
def __(alt, np, pd):
    def plot_validation_points(
        ...
        df = pd.DataFrame(
            {"X": X_valid.flatten(), "y": y_valid, "is_covered": is_covered}
        )
        points = (
            alt.Chart(df)
            ...
            .encode(
                ...
                tooltip=[alt.Tooltip("X:Q", title="X"), alt.Tooltip("y:Q", title="y")],
            )
        )
        return points
...
def __(
    ...
    plot_1a = plot_model_with_prediction_interval(
        model=model_1, X=X_plot_1, qhat=qhat_1
    ) + plot_validation_points(
    ...
    vertline_1 = (
        ...
        .mark_rule(strokeWidth=1, color="#D55E00", strokeDash=[6, 6])  # Dashed line
        .encode(
            x="x:Q",
        )
    )
    plot_1b = (
        plot_theoretical_finite_sample_coverage_pdf(
            alpha=alpha_1, calib_size=calib_size_1, valid_size=valid_size_1
        )
        + vertline_1
    )
...
def __(
    ...
    def sample_split_conformal_coverage_distribution(
        ...
        if training_conditional:
            X_train, y_train = sample_from_true_distribution(
                sample_size=train_size, rng=rng
            )
...
def __(
    ...
    def plot_observed_coverage_against_theory(
        ...
        obs_pmf: np.ndarray = (
            df_obs.groupby("num_covered").size().reset_index(name="pmf")
        )
        ...
        theo_pmf = pd.DataFrame({"num_covered": k_values, "pmf": probs})
    ...
    plot_observed_coverage_against_theory(
        ...
        height=400,
        title=[
            "Observed v Theoretical Coverage Distribution",
...
def __(alt, df_samples_2):
    alt.Chart(df_samples_2).mark_point(color="#D55E00").encode(
        x=alt.X(
            "num_covered:Q",
            title="Number of Covered Validation Samples",
            scale=alt.Scale(
    ...
    ).properties(title="Coverage vs Prediction Interval Size", width=400, height=300)
    return
